...
function pinballMapSearchName(name) {
  return baseMachineName(name)
    .replace(/\s+(?:Pro|Premium|LE|SE)$/i, "")
    .trim();
}

const pinballMapMachineIdCache = new Map();

async function fetchPinballMapMachineId(machineName) {
  const cacheKey = normalizeMachineTitle(machineName);
  if (pinballMapMachineIdCache.has(cacheKey)) return pinballMapMachineIdCache.get(cacheKey);

  const searchTerm = encodeURIComponent(pinballMapSearchName(machineName));
  const url = `${pinballMapConfig.siteBaseUrl}/api/v1/machines.json?by_name=${searchTerm}`;

  try {
    const response = await fetch(url);
    if (!response.ok) { pinballMapMachineIdCache.set(cacheKey, null); return null; }

    const payload = await response.json();
    const machines = Array.isArray(payload?.machines) ? payload.machines : [];
    const normalizedSearch = normalizeMachineTitle(machineName);

    let bestId = null;
    let bestScore = 0;

    for (const m of machines) {
      const mNorm = normalizeMachineTitle(m.name || "");
      if (mNorm === normalizedSearch) {
        const score = 100 + ((m.name || "").toLowerCase().includes("pro") ? 10 : 0);
        if (score > bestScore) { bestScore = score; bestId = m.id; }
      } else if (mNorm.includes(normalizedSearch) || normalizedSearch.includes(mNorm)) {
        const score = 20 + ((m.name || "").toLowerCase().includes("pro") ? 5 : 0);
        if (score > bestScore) { bestScore = score; bestId = m.id; }
      }
    }

    pinballMapMachineIdCache.set(cacheKey, bestId);
    return bestId;
  } catch {
    pinballMapMachineIdCache.set(cacheKey, null);
    return null;
  }
}

async function fetchLocationsByMachineId(machineId, origin, maxMiles) {
  const lat = encodeURIComponent(String(origin.latitude));
  const lon = encodeURIComponent(String(origin.longitude));
  const distance = encodeURIComponent(String(Math.ceil(Number(maxMiles || 50))));
  const url = `${pinballMapConfig.siteBaseUrl}/api/v1/locations.json?by_machine_id=${machineId}&by_lat_lon=${lat},${lon}&max_distance=${distance}&in_distance_type=m`;

  try {
    const response = await fetch(url);
    if (!response.ok) return [];
    const payload = await response.json();
    return normalizeLocationCollection(payload);
  } catch {
    return [];
  }
}

export async function findNearbyPinballMapLocations({ zip, maxMiles = 50, targetMachineNames = [] }) {
  const origin = await geocodeUsZip(zip);

  // Parallel: radius location discovery + machine-targeted search for each shortlist machine.
  // The machine-first searches are guaranteed accurate: if Pinball Map says a venue has
  // Jaws (via by_machine_id), it definitely does — no inference from venue→machine mapping.
  const [allRaw, ...machineSearchResults] = await Promise.all([
    fetchUniqueNearbyLocations(origin, maxMiles),
    ...targetMachineNames.slice(0, 6).map(async (name) => {
      const machineId = await fetchPinballMapMachineId(name);
      if (!machineId) return null;
      const locations = await fetchLocationsByMachineId(machineId, origin, maxMiles);
      const slug = bestInternalSlugForMachineName(name);
      return { slug, locations };
    })
  ]);

  // Build guaranteed machine attribution: locationId → Set<slug>
  const guaranteedSlugs = new Map();
  const extraLocationMap = new Map();
  const allRawIds = new Set(allRaw.map((l) => String(l.id)));

  for (const result of machineSearchResults) {
    if (!result?.slug) continue;
    for (const loc of result.locations) {
      const id = String(loc.id || "");
      if (!id) continue;
      const slugSet = guaranteedSlugs.get(id) || new Set();
      slugSet.add(result.slug);
      guaranteedSlugs.set(id, slugSet);
      if (!allRawIds.has(id)) extraLocationMap.set(id, loc);
    }
  }

  // Merge radius results with any locations only surfaced by machine-first search.
  const mergedRaw = [...allRaw, ...extraLocationMap.values()];
  const rawLocations = sortRawLocationsByProximity(mergedRaw, origin).slice(0, pinballMapConfig.locationSearchLimit);

  if (!rawLocations.length) {
    return [];
  }

  const machineDetailBudget = pinballMapConfig.machineDetailLimit ?? 40;
  ...
  const enriched = await batchProcess(rawLocations, async (location) => {
    ...
    let machineIds = [];
    const rawNames = location.machine_names;
    const machineNames = Array.isArray(rawNames)
    ...
    if (!machineIds.length && !machineNames.length && machineDetailCallsUsed < machineDetailBudget) {
      ...
      try {
        ...
        console.warn("[Pinball Scout] machine detail lookup failed", {
    ...
    // Merge any guaranteed slugs from machine-first search (authoritative Pinball Map data).
    const guaranteed = guaranteedSlugs.get(String(location.id));
    if (guaranteed) {
      guaranteed.forEach((slug) => machineIds.push(slug));
      machineIds = [...new Set(machineIds)];
    }
