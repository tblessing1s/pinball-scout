...
import { BUYER_CHECKLIST } from "../../../data/buyer-guide.js";
import { buildPinsideMarketUrl, buildPinsidePricingUrl } from "../../services/pinside-market.js";
...
export function renderSourcingForm() {
  ...
  const pricing = pricingSummary(machine, marketPricingIndex);
  const marketUrl = buildAffiliateUrl(buildPinsideMarketUrl(machine), { machineSlug: machine.slug, placement: "sourcing-listings" });
  const pricingUrl = buildAffiliateUrl(buildPinsidePricingUrl(machine), { machineSlug: machine.slug, placement: "sourcing-pricing" });

  const pricingPanel = `
    <article class="panel pricing-reality">
      <p class="eyebrow">Market pricing</p>
      <h3>What to expect to pay</h3>
      ${pricing.used ? `
        <p><strong>Used market:</strong> ${pricing.used.rangeText} <span class="muted">(${pricing.used.medianText})</span></p>
        <p class="muted">${pricing.used.detailText}</p>
      ` : ""}
      <p><strong>${pricing.estimated.label}:</strong> ${pricing.estimated.rangeText}</p>
      <p class="muted">${pricing.estimated.detailText}</p>
      ${budgetContextNote(machine)}
      <div class="card-actions">
        <a class="btn btn-secondary small" href="${pricingUrl}" target="_blank" rel="noreferrer"
           data-outbound-track data-outbound-label="Pricing history" data-outbound-slug="${machine.slug}"
           data-outbound-placement="sourcing-pricing">See pricing history</a>
      </div>
    </article>
  `;

  const listingPanel = `
    <article class="panel find-listing">
      <p class="eyebrow">Find one to buy</p>
      <h3>Where to look for a ${machine.name}</h3>
      <div class="card-actions">
        <a class="btn btn-primary" href="${marketUrl}" target="_blank" rel="noreferrer"
           data-outbound-track data-outbound-label="Used listings" data-outbound-slug="${machine.slug}"
           data-outbound-placement="sourcing-listings">Browse used listings</a>
        ${machine.new_or_used_availability !== "Used" ? `
          <a class="btn btn-secondary" href="${buildAffiliateUrl("https://pinside.com/pinball/trade/wanted", { machineSlug: machine.slug, placement: "sourcing-dealer" })}" target="_blank" rel="noreferrer"
             data-outbound-track data-outbound-label="New from dealer" data-outbound-slug="${machine.slug}"
             data-outbound-placement="sourcing-dealer">Find new from dealer</a>
        ` : ""}
      </div>
      <p class="muted">Links open in Pinside — the largest pinball marketplace.</p>
    </article>
  `;

  const machineSpecificCautions = machineProfile.what_to_know_before_buying?.length ? `
    <div class="detail-list-block">
      <p><strong>${machine.name} — things to know before buying</strong></p>
      <ul class="result-list">
        ${machineProfile.what_to_know_before_buying.map((item) => `<li>${item}</li>`).join("")}
      </ul>
    </div>
  ` : "";

  const checklistPanel = `
    <article class="panel buyers-checklist">
      <div class="section-head compact">
        <div>
          <p class="eyebrow">First-time buyer guide</p>
          <h3>What to look for</h3>
        </div>
        <button class="btn btn-secondary small" type="button" data-action="toggle-checklist">
          ${state.checklistOpen ? "Hide guide" : "Show guide"}
        </button>
      </div>
      ${state.checklistOpen ? `
        ${machineSpecificCautions}
        ${Object.values(BUYER_CHECKLIST).map((section) => `
          <details>
            <summary><strong>${section.label}</strong></summary>
            <ul class="result-list">${section.items.map((item) => `<li>${item}</li>`).join("")}</ul>
          </details>
        `).join("")}
      ` : `<p class="muted">A structured guide covering listing photos, seller questions, in-person checks, red flags, and transaction tips.</p>`}
    </article>
  `;
