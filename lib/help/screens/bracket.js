...
export function renderBracketTestGroup(targetMachine, rankIndex, allSuggestions, rankedProxyTraitChecks) {
  ...
  const probes = hasExactWithLocation
    ...
    : allProbes.filter((s) => s.locationMatches.length > 0).slice(0, 3);
  const focusChecks = rankedProxyTraitChecks(targetMachine, 3).map((item) => item.prompt);
  const availableProbes = allProbes.filter((s) => s.locationMatches.length > 0);
  const playedCount = availableProbes.filter((s) =>
    (state.reactions || []).some((r) => r.machineId === s.machineId && String(r.source || "").startsWith("real-play"))
  ).length;
  ...
  const renderProbeRow = (suggestion) => {
    ...
    const markedUnavailable = (state.reactions || []).some(
      (r) => r.machineId === suggestion.machineId && r.experienceType === "unavailable"
    );
    return `
      <div class="bracket-probe-row${alreadyPlayed ? " is-played" : ""}${markedUnavailable ? " is-unavailable" : ""}">
        ...
        <div class="bracket-probe-body">
          ...
          ${(() => {
            const alsoValidates = suggestion.targetIds
              .filter((id) => id !== targetMachine.id)
              .map((id) => discoveryMachineIndex.get(id)?.name)
              .filter(Boolean);
            return alsoValidates.length
              ? `<p class="bracket-probe-hint">Also validates: ${alsoValidates.join(", ")}</p>`
              : suggestion.probeType !== "exact"
              ? `<p class="bracket-probe-hint">When playing this, notice: ${focusChecks[0] || "how the game feel compares to what you want to own"}</p>`
              : "";
          })()}
        </div>
        <div class="bracket-probe-action">
          ${alreadyPlayed
            ...
            : markedUnavailable
            ? `<span class="played-badge played-badge--unavailable">Unavailable ✗</span>`
            : `<div class="bracket-probe-actions">
                <button class="btn btn-primary small" type="button" data-action="log-play:${suggestion.machineId}">I played this</button>
                <button class="btn btn-ghost small" type="button" data-action="log-unavailable:${suggestion.machineId}">Unavailable</button>
              </div>`
          }
        </div>
      </div>
  ...
  return `
    <div class="bracket-test-group panel">
      <div class="bracket-test-group__header">
        ...
        <div>
          <p class="eyebrow">${meta.rank}${availableProbes.length > 0 ? ` · <span class="badge ${playedCount > 0 ? "badge--success" : ""}">${playedCount}/${availableProbes.length} played</span>` : ""}</p>
