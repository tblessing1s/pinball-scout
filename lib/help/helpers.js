...
export function pricingSummary(machine, marketPricingIndex = null) {
  if (typeof utils.machinePricingSummary === "function") {
    return utils.machinePricingSummary(machine);
  }
  const marketData = marketPricingIndex?.get(machine?.slug || machine?.id);
  const used = marketData?.usedRange ? {
    label: "Used market range",
    rangeText: `${formatCurrency(marketData.usedRange.min)}–${formatCurrency(marketData.usedRange.max)}`,
    medianText: `median ${formatCurrency(marketData.usedRange.median)}`,
    confidence: marketData.confidence,
    sampleSize: marketData.sampleSize,
    detailText: `Based on ${marketData.sampleSize} recent sales${marketData.pinballPricesUpdateLabel ? ` · ${marketData.pinballPricesUpdateLabel}` : ""}`
  } : null;
  return {
    estimated: {
      label: "New / catalog estimate",
      rangeText: `${formatCurrency(machine.estimated_price_min)}–${formatCurrency(machine.estimated_price_max)}`,
      detailText: "Catalog estimate — may vary by version and dealer"
    },
    used
  };
}
