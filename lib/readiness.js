...
function tasteSignal({ machineSlug, reactions = [], recommendations = [], context = {}, refinementSignals = [] }) {
  const uniqueReactions = new Set(reactions.map((reaction) => reaction.machineId)).size;
  const confidence = recommendationConfidence(context, reactions, recommendations, { refinementSignals });
...
export function buildReadinessModel({
  ...
  const cards = [
    tasteSignal({ machineSlug, reactions, recommendations, context, refinementSignals }),
    practicalSignal({ machineSlug, context, pathType }),
    validationSignal({ machineSlug, pathType, reactions, refinementSignals, requiredCheckProgress, firstUnresolvedBlockerId })
  ];
