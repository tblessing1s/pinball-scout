...
function renderBudgetCheck() {
  ...
  return `
    <section class="discovery-shell">
      ${renderProgress(
        "What's your budget?",
        "Tap one — we'll rank your matches within reach and show your results.",
        68
      )}
...
const TASTE_PIVOT_PAIRS = [
  {
    question: "When you first sit down at a new machine, what matters more?",
    left: {
      machineId: "deadpool-pro",
      details: [
        "Cracking up within the first ball — clear targets and one-liners reward you fast",
        "Short, energetic games that keep pulling you back for another go"
      ],
      tasteAnswers: {
    ...
    right: {
      ...
      details: [
        "Big story moments that unlock progressively — always chasing the next dinosaur sequence",
        "Still finding new shots and strategies after 50 games — the more you learn, the more it gives"
      ],
      tasteAnswers: {
  ...
  },
  {
    ...
    left: {
      ...
      details: [
        "You're literally inside the show — Demogorgons, the Mind Flayer, and Eleven are all there",
        "Any guest can walk up and immediately know what they're trying to do"
      ],
      tasteAnswers: {
    ...
    right: {
      ...
      details: [
        "Tight, fast layout built around nailing a great set of precise, satisfying shots",
        "Rewards you for getting better — becomes a different machine once you learn to aim"
      ],
      tasteAnswers: {
  ...
  },
  {
    ...
    left: {
      ...
      details: [
        ...
        "Works for the whole family and any guest, not just serious players"
      ],
      tasteAnswers: {
    ...
    right: {
      ...
      details: [
        "Deep modes and big multiball sequences that reward you for figuring out the rules",
        "Still discovering new strategies months after you bring it home"
      ],
      tasteAnswers: {
        "learning-curve": "deep-discovery",
        "replay-itch": "longer-progress",
...
function renderTastePivot() {
  ...
  const progress = 70 + Math.round((pairIndex / TASTE_PIVOT_PAIRS.length) * 10);

  return `
    <section class="discovery-shell">
      ${renderProgress(
        "What sounds most like fun?",
        pair.question,
        progress,
...
async function advanceReaction(rawReaction) {
  ...
  if (state.deckIndex >= state.deck.length - 1) {
    ...
    if (state.mode !== "calibration") state.tastePivotIndex = 0;
...
root?.addEventListener("click", (event) => {
  ...
  if (action?.startsWith("taste-pivot-pick:")) {
    ...
    state.tasteAnswers = { ...state.tasteAnswers, ...chosen.tasteAnswers };
