...
function renderBracketNearby() {
  ...
  const nearbyMatches = hasLocationSearch ? getNearbyTasteMatches(finalThree) : [];
  ...
  const renderFinalistCard = (machine, rank) => {
    if (!machine) return "";
    const isPlayed = state.selectedPlayedMachines.includes(machine.id);
    const isOwned = state.ownedMachines.includes(machine.id);
    return `
      <article class="panel bracket-finalist-card">
        ${renderMachineImage(machine, { eager: true, className: "machine-image-frame--thumb" })}
        <div class="bracket-finalist-body">
          <p class="eyebrow">${rank === 1 ? "🏆 Your champion" : rank === 2 ? "Runner-up" : "3rd place"}</p>
          <h3>${machineDisplayTitle(machine)}</h3>
          <p class="bracket-finalist-why">${buildMachineArchetypeReason(machine, archetype, state.tasteScores)}</p>
          ${formatPrice(machine) ? `<p class="muted bracket-finalist-price">${formatPrice(machine)}</p>` : ""}
          <div class="bracket-finalist-badges">
        ...
        ${rank === 1
          ...
          : `<button class="btn btn-secondary small" type="button" data-action="open-sourcing:${machine.id}">Use this instead</button>`
        }
      </article>
    `;
  };
  ...
  const renderBudgetPicker = () => `
    <div class="panel bracket-budget-picker">
      <h3>What's your budget?</h3>
      <p class="muted">Helps us prioritize what to focus on in your buying plan.</p>
      <div class="budget-band-grid">
        ${BUDGET_BANDS.map((band) => `
          <button
            class="budget-band-btn${selectedBudget === band.id ? " is-selected" : ""}"
            type="button"
            data-action="bracket-budget-set:${band.id}"
          >
  ...
  const renderNearbySection = () => {
    if (!hasLocationSearch) {
      return `
        <div class="panel bracket-nearby-zip">
          <h3>Can you play your champion nearby?</h3>
          <p class="muted">Search Pinball Map to see if ${champion ? machineDisplayTitle(champion) : "your top pick"} — or something with a matching feel — is playable near you before you buy.</p>
          ...
          <p class="muted small">Or <button class="btn-inline-link" type="button" data-action="bracket-nearby-buy">skip to buying plan</button> and search there.</p>
        </div>
      `;
    }
    if (!nearbyMatches.length) {
      return `
        <div class="panel bracket-nearby-empty">
          <h3>No catalog matches found nearby</h3>
          <p class="muted">The machines near you aren't in our current catalog, but your buying plan will help you track them down.</p>
        </div>
      `;
    }
    return `
      <div class="panel bracket-nearby-matches">
        <h3>Taste-matched machines near you</h3>
        <p class="muted">These are at locations you can visit — good for testing your taste profile before you buy.</p>
        <div class="bracket-nearby-grid">
          ${nearbyMatches.map((machine) => {
            const isPlayed = state.selectedPlayedMachines.includes(machine.id);
            const locs = state.locationSearch.machineLocationIndex?.get(machine.id) || [];
            const nearestLoc = locs[0];
            return `
              <article class="bracket-nearby-card">
                ${renderMachineImage(machine, { eager: false, className: "machine-image-frame--thumb" })}
                <div class="bracket-nearby-card-body">
                  <h4>${machineDisplayTitle(machine)}</h4>
                  ${nearestLoc ? `<p class="muted small">${nearestLoc.name}${nearestLoc.distanceMiles != null ? ` · ${nearestLoc.distanceMiles} mi` : ""}</p>` : ""}
                  <p class="muted small">${machine.shortDescription || machine.description || ""}</p>
                </div>
                <button
                  class="btn ${isPlayed ? "btn-secondary" : "btn-outline"} small"
                  type="button"
                  data-toggle-played="${machine.id}"
                >${isPlayed ? "✓ Played" : "Mark played"}</button>
              </article>
            `;
          }).join("")}
        </div>
  ...
  return `
    <section class="discovery-shell">
      <div class="bracket-nearby-header">
        <h2>Your bracket results</h2>
        <p class="muted">Based on your taste profile, here's how the bracket finished.</p>
      </div>
      <div class="bracket-finalists">
        ${renderFinalistCard(champion, 1)}
        ${runnerUp || third ? `<div class="bracket-runners-up">${renderFinalistCard(runnerUp, 2)}${renderFinalistCard(third, 3)}</div>` : ""}
      </div>
      ${renderBudgetPicker()}
      ${renderNearbySection()}
      <div class="discovery-actions">
        <button class="btn btn-primary" type="button" data-action="bracket-nearby-buy">
          Start buying plan for ${champion ? machineDisplayTitle(champion) : "my pick"}
        </button>
        <button class="btn btn-secondary small" type="button" data-action="back-to-profile">
          Back to profile
        </button>
      </div>
    </section>
...
function renderTastePivot() {
  ...
  return `
    <section class="discovery-shell">
      ...
      <div class="taste-pivot-grid">
        <article class="taste-pivot-card panel">
          ...
          <div class="taste-pivot-body">
            ...
            <ul class="taste-pivot-details">
              ${pair.left.details.map((d) => `<li>${d}</li>`).join("")}
            </ul>
            <button class="taste-pivot-why-btn" type="button" data-action="taste-pivot-details:${pairIndex}:left">Why this? →</button>
          </div>
          <button class="btn btn-primary" type="button" data-action="taste-pivot-pick:${pairIndex}:left">This sounds like me</button>
        </article>
        ...
        <article class="taste-pivot-card panel">
          ...
          <div class="taste-pivot-body">
            ...
            <ul class="taste-pivot-details">
              ${pair.right.details.map((d) => `<li>${d}</li>`).join("")}
            </ul>
            <button class="taste-pivot-why-btn" type="button" data-action="taste-pivot-details:${pairIndex}:right">Why this? →</button>
          </div>
          <button class="btn btn-primary" type="button" data-action="taste-pivot-pick:${pairIndex}:right">This sounds like me</button>
        </article>
      </div>
      ${hasAnyScore ? `
...
root?.addEventListener("click", (event) => {
  ...
  if (action?.startsWith("taste-pivot-details:")) {
    ...
    el.setAttribute("data-action", "pivot-sheet-close");
    el.innerHTML = `
      <div class="modal-panel what-counts-sheet pivot-details-sheet" role="dialog" aria-modal="true">
        <p class="eyebrow">Why this machine?</p>
        <h3 class="pivot-sheet-title">${machineDisplayTitle(machine)}</h3>
        <ul class="taste-pivot-details taste-pivot-details--open">
    ...
    el.querySelector(".pivot-details-sheet").addEventListener("click", (e) => e.stopPropagation());
    root.appendChild(el);
    return;
  }
