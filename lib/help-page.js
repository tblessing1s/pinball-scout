...
import { buildComponentProxySuggestions, buildNearbyCatalogSuggestions, buildNearbyExternalMachineSuggestions, buildTasteProbeSuggestions } from "./test-plan-engine.js";
...
async function preloadNearbyContext() {
  ...
  try {
    const preloadRecs = currentRecommendations();
    const targetMachineNames = preloadRecs.map((m) => m.name || m.title).filter(Boolean);
    const locations = await findNearbyLocations({ zip, maxMiles, targetMachineNames });
    ...
    const tasteSuggestions = buildTasteProbeSuggestions(preloadRecs, state.context, machineLocationIndex);
    const componentProxies = buildComponentProxySuggestions(preloadRecs, machineLocationIndex, { maxSuggestions: 4 });
    state.nearbySuggestions = [
      ...tasteSuggestions,
      ...componentProxies.filter((cp) => !tasteSuggestions.some((s) => s.machineId === cp.machineId))
    ];
    state.nearbyCatalogSuggestions = buildNearbyCatalogSuggestions(preloadRecs, machineLocationIndex);
...
async function advanceReaction(rawReaction) {
  ...
  if (state.deckIndex >= state.deck.length - 1) {
    ...
    if (state.mode === "calibration") {
      ...
      syncDecisionStateFromRecommendations("", "help_calibration_complete");
    } else {
...
async function searchNearbyLocations(form) {
  ...
  try {
    ...
    const targetMachineNames = recommendations.map((m) => m.name || m.title).filter(Boolean);
    const locations = await findNearbyLocations({ zip, maxMiles, targetMachineNames });
    const machineLocationIndex = buildMachineLocationIndex(locations);
    state.locationSearch = { zip, maxMiles, locations, hasSearched: true, error: "", machineLocationIndex };
    ...
    const filteredSuggestions = bracketFinalSet.size > 0
      ? allSuggestions.filter((s) => !bracketFinalSet.has(s.machineId) || s.targetIds.includes(s.machineId))
      : allSuggestions;
    const componentProxies = buildComponentProxySuggestions(recommendations, machineLocationIndex, { maxSuggestions: 4 });
    state.nearbySuggestions = [
      ...filteredSuggestions,
      ...componentProxies.filter((cp) => !filteredSuggestions.some((s) => s.machineId === cp.machineId))
    ];
...
function submitPostPlayFeedback() {
  ...
  if (state.postPlayQueue?.length > 0) {
    const [nextId, ...remaining] = state.postPlayQueue;
    state.postPlayQueue = remaining;
    captureRefinementBaseline("played");
    state.activeProbeId = nextId;
    state.screen = "post-play-feedback";
  } else {
    state.postPlayQueue = [];
    state.screen = state.postPlayReturnScreen || "results";
    state.postPlayReturnScreen = "";
  }
...
root?.addEventListener("click", (event) => {
  ...
  if (action === "taste-track-played") {
    state.preLogRecsSnapshot = currentRecommendations().map((m) => m.id);
  ...
  if (action === "taste-played-done") {
    ...
    const reactedIds = new Set(state.reactions.map((r) => r.machineId));
    const unratedPlayed = state.selectedPlayedMachines.filter((id) => recIds.has(id) && !reactedIds.has(id));

    if (unratedPlayed.length > 0) {
      const existingProbeIds = new Set([...state.nearbySuggestions, ...state.nearbyCatalogSuggestions].map((p) => p.machineId));
      const newProbes = unratedPlayed
        .filter((id) => !existingProbeIds.has(id))
        .map((id) => {
          const machine = recs.find((m) => m.id === id);
          return {
            machineId: id, targetIds: [id], probeType: "exact",
            reason: `You've played ${machine?.name || "this machine"} — how did it feel?`,
            reveals: "Your reaction directly adjusts your shortlist rankings.",
            machine, locationMatches: state.locationSearch.machineLocationIndex?.get(id) || []
          };
        });
      if (newProbes.length) state.nearbySuggestions = [...state.nearbySuggestions, ...newProbes];
      state.postPlayQueue = unratedPlayed.slice(1);
      captureRefinementBaseline("played");
      state.activeProbeId = unratedPlayed[0];
      resetDraft();
      state.screen = "post-play-feedback";
    } else {
      const newIds = recs.map((m) => m.id);
  ...
  if (action === "toggle-checklist") { state.checklistOpen = !state.checklistOpen; render(); return; }
  ...
  if (action?.startsWith("log-unavailable:")) { const machineId = action.split(":")[1]; state.reactions.push({ machineId, reaction: "unavailable", source: "real-play-skip", weight: 0, experienceType: "unavailable" }); render(); return; }
  if (action === "go-to-results") { state.screen = "results"; render(); return; }
  ...
  if (action === "restart") {
    ...
    Object.assign(state, {
      ...
      activeProbeId: "", postPlayQueue: [], sourcingMachineId: "",
