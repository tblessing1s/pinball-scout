...
async function preloadNearbyContext() {
  ...
  try {
    ...
    const componentProxies = buildComponentProxySuggestions(preloadRecs, machineLocationIndex);
...
async function searchNearbyLocations(form) {
  ...
  try {
    ...
    const filteredSuggestions = bracketFinalSet.size > 0
      ...
      : allSuggestions;
    const componentProxies = buildComponentProxySuggestions(recommendations, machineLocationIndex);
    state.nearbySuggestions = [
      ...filteredSuggestions,
