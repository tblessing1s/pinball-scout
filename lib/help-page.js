...
import { clearDecisionState, loadDecisionState, saveDecisionState } from "./decision-state.js";
...
function applySavedSession(saved) {
  ...
  state.screen = saved.screen || "entry";
...
function renderEntryScreen() {
  ...
  const compareCard = comparedMachines?.length
    ? `
      <article class="panel">
        <p class="eyebrow">Compare handoff</p>
        <h3>Continue your compare decision</h3>
        <p class="muted">You were comparing ${comparedMachines.join(" vs ")}. Use this guided flow to break the tie with a clearer next step.</p>
      </article>
    `
    : "";
  const resumeCard = state.hasSavedSession
    ? `
      <article class="panel">
        <p class="eyebrow">Resume your progress</p>
        <h3>You have a saved shortlist session</h3>
        <p class="muted">Continue where you left off, including your reactions and current recommendation progress.${savedAtLabel ? ` Last saved ${savedAtLabel}.` : ""}</p>
        <div class="discovery-actions">
          <button class="btn btn-primary" type="button" data-action="resume-saved">Resume session</button>
          <button class="btn btn-secondary" type="button" data-action="discard-saved">Start fresh</button>
        </div>
      </article>
    `
    : "";
  ...
  return `
    <section class="discovery-shell">
      ${renderProgress(
        "Welcome to your first-pin concierge",
        "We start with practical guardrails, then learn from your reactions, then deliver a decision-ready shortlist.",
        10
      )}
      ${compareCard}
      ${resumeCard}
      <div class="choice-grid">
        <button class="choice-card" type="button" data-entry="no">
          <strong>I have played very little</strong>
          <span>Start broad and build confidence with curated beginner-safe machine previews.</span>
        </button>
        <button class="choice-card" type="button" data-entry="yes">
          <strong>I have played enough to calibrate</strong>
          <span>Use a faster path based on machines you already recognize.</span>
        </button>
      </div>
    </section>
  `;
...
function renderTasteSummary() {
  ...
  return `
    <article class="panel taste-summary">
      ...
      <h3>Your early preferences</h3>
      <p class="muted">These quick choices shape the first round of machine previews.</p>
      <div class="taste-chip-row">
        ${selections.length ? selections.map((label) => `<span class="badge">${label}</span>`).join("") : `<span class="muted">Make a few picks to see your taste profile come alive.</span>`}
      </div>
    </article>
  `;
}
...
function renderDiscoveryRound() {
  ...
  return `
    <section class="discovery-shell">
      ${renderProgress(
        "Stage 2: Guided machine reactions",
...
function renderResults() {
  ...
  const topRecommendationMarketUrl = topRecommendation ? (topRecommendation.externalLinks?.pinsideMarket || buildPinsideMarketUrl(topRecommendation)) : "";
  ...
  return `
    <section class="discovery-shell">
      ...
      <article class="panel">
        ...
        <p class="muted"><strong>How this is calculated:</strong> reaction signal + practical constraints + validation readiness.</p>
        ${isLowConfidence || isMediumConfidence ? `
          ...
          <p class="muted">${confidence.nextSteps.map((step) => step.detail).join(" ")}</p>
        ` : ""}
      </article>
      ...
      <article class="panel results-next-step">
        ...
        <div class="card-actions">
          <button class="btn btn-primary" type="button" data-action="open-sourcing:${topRecommendation?.id || ""}">Finalize My Buying Plan</button>
          <button class="btn btn-secondary" type="button" data-action="open-nearby">Find places to test</button>
        </div>
      </article>
      <div class="discovery-results-grid">
        ${framed.map((item) => {
          ...
          return renderSplitCard({
            ...
            mediaClassName: "recommendation-card__media",
            bodyClassName: "recommendation-card__body",
            overlayHtml: `<div class="recommendation-rank">${isLowConfidence ? "Candidate" : "Fit"}</div>`,
            ...
            <ul class="result-list">
              ${(explanation.whyMatch || machine.whyItFits || []).map((reason) => `<li>${reason}</li>`).join("")}
            </ul>
            <p><strong>Why beginners choose it</strong></p>
            <ul class="result-list">
              ${explanation.beginnerWhy ? `<li>${explanation.beginnerWhy}</li>` : ""}
              <li>Beginner-friendliness: ${beginnerLabel(machine.beginnerFriendly)}</li>
              <li>Resale strength: ${resaleLabel(machine.resaleStrength)}</li>
              <li>Maintenance complexity: ${maintenanceLabel(machine.maintenanceComplexity)}</li>
            </ul>
            <p><strong>Possible downside</strong></p>
            <p class="muted">${explanation.downside || whatToKnow[0] || machine.cautionNote || "Make sure the game feel still matches after a real play test."}</p>
            <p><strong>What to confirm before buying</strong></p>
            <p class="muted">${explanation.confirmInPerson || machine.validationPlan}</p>
            <p><strong>What you seem to like</strong></p>
            <div class="discovery-tag-row">
              ${likelyFitTags.map((tag) => `<span class="badge">${tag}</span>`).join("")}
            </div>
            <p class="fit-summary"><strong>Confidence summary:</strong> ${machine.confidenceBlurb}</p>
            ...
            <div class="card-actions">
              ...
              <button class="btn btn-primary small" type="button" data-action="open-sourcing:${machine.id}">Use this as my front-runner</button>
            </div>
            `
          });
        }).join("")}
      ...
      <article class="panel more-actions-panel">
        ...
        ${state.moreActionsOpen ? `
          <div class="card-actions">
            ...
            ${topRecommendation ? `<a class="btn btn-secondary" href="${topRecommendationMarketUrl}" target="_blank" rel="noreferrer">Open used listings</a>` : ""}
          </div>
...
function renderSourcingForm() {
    ...
    return `
      <section class="discovery-shell">
        ...
        <article class="panel">
          ...
          <div class="discovery-actions">
            <button class="btn btn-primary" type="button" data-action="back-to-results">Back to recommendations</button>
          </div>
        </article>
      </section>
    `;
  ...
  const continuityPanel = `
    <article class="panel plan-continuity-panel">
      <p class="eyebrow">Decision continuity</p>
      <h3>Active machine: ${displayTitle}${decisionState.temporaryPrimary ? " (Temporary)" : ""}</h3>
      <p class="muted">${decisionState.temporaryPrimary
        ? "You’re in temporary-primary mode. Use the next validation step to confirm or switch before purchase."
        : "This machine is currently your front-runner across results, compare, and buying plan."
      }</p>
      ...
      ${continuity.backupMoreReady && continuity.topBackup ? `
        <article class="decision-alert">
          ...
          <p class="muted">Why: ${continuity.reasonCodes.join(", ").replaceAll("_", " ")}.</p>
          <div class="card-actions">
            ...
            <button class="btn btn-secondary small" type="button" data-action="plan-keep-fix">Keep current and fix blockers</button>
          </div>
        </article>
      ` : ""}
    </article>
  ...
  const stepBody = step === 1
    ? `
      <article class="panel plan-step-panel${state.activePlanFocus === "working-pick-step" ? " is-focused" : ""}" id="plan-working-pick-step">
        ...
        <div class="discovery-actions">
          <button class="btn btn-primary" type="button" data-action="plan-keep-pick">Keep this working pick</button>
    ...
    : step === 2
      ? `
        <article class="panel plan-step-panel${state.activePlanFocus === "fit-risk-step" ? " is-focused" : ""}" id="plan-fit-risk-step">
          ...
          <p><strong>Why this fits you</strong></p>
          <ul class="result-list">
            ${whyItFits.map((item) => `<li>${item}</li>`).join("")}
          </ul>
      ...
      : step === 3
        ...
        : `
          <article class="panel plan-step-panel">
            ...
            ${riskModel.primary ? `
              <article class="risk-banner risk-banner--${riskModel.primary.type}">
                <div class="risk-banner__head">
                  ...
                  ${riskModel.secondary?.length ? `<button class="chip-button" type="button" data-action="risk-drawer-toggle">${state.activeRiskDrawer ? "Hide extra risk" : `+${riskModel.secondary.length} additional risk`}</button>` : ""}
                </div>
                ...
                ${state.activeRiskDrawer && riskModel.secondary?.length ? `
                  <div class="risk-secondary-drawer">
                    ${riskModel.secondary.map((risk) => `<p class="muted"><strong>${risk.title}</strong> ${risk.body}</p>`).join("")}
                  </div>
                ` : ""}
              </article>
            ` : ""}
            ...
            <div class="discovery-actions">
              <button class="btn btn-primary" type="button" data-action="plan-finish">Save plan and return to shortlist</button>
              <button class="btn btn-secondary" type="button" data-action="plan-prev-step">Back</button>
            </div>
          </article>
        `;
...
root?.addEventListener("click", (event) => {
  ...
  if (action === "resume-saved") {
    const saved = loadPersistedDiscoveryState();
    if (!applySavedSession(saved)) return;
    trackAnalytics(ANALYTICS_EVENTS.RESUME_BANNER_CLICKED, {
      state_type: "saved_session",
      screen: state.screen
    });
    render();
    return;
  ...
  if (action === "plan-finish") {
    ...
    showSiteMessage("Buying plan saved. You can continue validating or compare backups.");
    state.screen = "results";
    state.activeWhatCountsId = "";
    state.activeRiskDrawer = false;
    state.activePlanFocus = "";
  ...
  if (action === "plan-keep-fix") {
    ...
    render();
    window.setTimeout(() => {
      root.querySelector("[data-required-checks]")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 40);
    return;
  }
  ...
  if (action?.startsWith("readiness-action:")) {
    ...
    const activePlan = currentPlanState();
    ...
    if (target === "path-step") {
      ...
      render();
      window.setTimeout(() => {
        root.querySelector("#plan-path-step")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 40);
      return;
    }
    if (target === "required-checks") {
      ...
      render();
      window.setTimeout(() => {
        const selector = blockerId ? `#required-check-${blockerId}` : "[data-required-checks]";
    ...
    if (target === "fit-risk-step") {
      ...
      render();
      window.setTimeout(() => {
        root.querySelector("#plan-fit-risk-step")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 40);
      return;
    }
    if (target === "working-pick-step") {
      ...
      render();
      window.setTimeout(() => {
        root.querySelector("#plan-working-pick-step")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 40);
      return;
    }
  }
...
state.hasSavedSession = Boolean(initialSavedSession) || Boolean(decisionState.hasLegacyDiscovery);
state.savedSessionUpdatedAt = initialSavedSession?.updatedAt || decisionState.updatedAt || "";
...
decisionBarController = initDecisionBar({
  ...
  handlers: {
    ...
    fixBlockers: () => {
      state.screen = "sourcing";
      state.sourcingMachineId = currentRecommendations().find((machine) => machine.slug === loadDecisionState().frontRunnerSlug)?.id || state.sourcingMachineId;
      render();
      window.setTimeout(() => {
        root.querySelector("[data-required-checks]")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 40);
...
if (pageParams.get("resume") === "1" && initialSavedSession) {
  applySavedSession(initialSavedSession);
  trackAnalytics(ANALYTICS_EVENTS.RESUME_BANNER_CLICKED, {
    state_type: "saved_session",
    screen: state.screen
  });
