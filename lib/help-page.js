...
function buildMachineArchetypeReason(machine, archetype) {
  const prefixes = {
    "easy-rider": "Easy to love from ball one —",
    "flow-chaser": "Built for players who live in the zone —",
    "theme-collector": "Theme depth you'll lose yourself in —",
    "serious-player": "A craftsman's machine —",
    "safe-starter": "Smart first-home pick —",
    "deep-explorer": "Deep enough to keep discovering for years —"
  };
  const prefix = prefixes[archetype.id] || "A strong match —";
  return `${prefix} ${machine.description || machine.best_for || ""}`;
}
...
function renderTastePlayedMachines() {
  return `
    <section class="discovery-shell">
      ${renderProgress(
        "Games you've played",
        "Select any machines you've played before — we'll prioritize them in future comparisons.",
        90
      )}
...
function renderTasteProfileReveal() {
  ...
  const recs = recommendMachines(currentContext(), [], 5, {
    machineLocationIndex: state.locationSearch.machineLocationIndex
  });
  ...
  return `
    <section class="discovery-shell">
      ...
      ${recs.length > 0 ? `
        <article class="panel">
          <p class="eyebrow">Machines for you</p>
            ...
            ${recs.map((machine) => `
              <div class="profile-rec-card">
                ...
                <div class="profile-rec-body">
                  ...
                  <p>${buildMachineArchetypeReason(machine, archetype)}</p>
                </div>
              </div>
            `).join("")}
          </div>
      ...
      <div class="discovery-actions">
        <button class="btn btn-primary" type="button" data-action="show-recommendations">See my full recommendations</button>
      </div>
    </section>
  `;
}
