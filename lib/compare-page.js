...
function canonicalSelection(decisionState) {
  ...
  const chosen = storeSlugs.length ? storeSlugs : stateSlugs.length ? stateSlugs : decisionSlugs;
...
function compareReasonLine(baseRanked, activeRanked, presetMode) {
  ...
  if (presetMode === "balanced") return "Balanced mode is active. Ranking favors beginner safety and stable ownership.";
  if (baseWinner.machine.slug === activeWinner.machine.slug) return "Winner unchanged under this priority.";
  ...
  return `Switched to ${machineDisplayTitle(activeWinner.machine)} because ${labels[winnerContrib]} now outweighs ${labels[loserContrib]}.`;
}
  ...
  root.innerHTML = `
    <div class="panel empty-state compare-empty-state">
      ...
      <div class="hero-actions compare-empty-actions">
        <a class="btn btn-primary" href="help.html?resume=1">Return to your recommendations</a>
...
function render() {
  ...
  syncDecisionStateFromCompare(selected, { selectedSlugs: selectionSlugs });
  updateCompareCount();
  ...
  const temporaryBanner = decisionState.temporaryPrimary && decisionState.frontRunnerSlug
    ? `<article class="panel compare-reco-banner is-neutral"><h3>Temporary front-runner: ${machineDisplayTitle(machineBySlug(decisionState.frontRunnerSlug) || { name: decisionState.frontRunnerSlug, title: decisionState.frontRunnerSlug })}</h3><p class="muted">Tie-breaker unresolved. Recheck after next validation.</p><a class="btn btn-secondary small" href="help.html?resume=1">Continue validation plan</a></article>`
    : "";

  const winnerBanner = pairOutcome.tooClose
    ? `<article class="panel compare-reco-banner is-neutral"><h3>Too close to call right now</h3><p class="muted">Both options are still strong. Run the tie-breaker to resolve the final gap.</p><button class="btn btn-primary small" type="button" data-action="tie-breaker-start">Run tie-breaker and re-rank</button></article>`
  ...
  const dualReadyStrip = pairOutcome.tooClose && pairOutcome.bothReady && winner && runnerUp ? (() => {
    const safer = pairOutcome.tradeoff.saferSlug === winner.machine.slug ? winner.machine : runnerUp.machine;
    const exciting = pairOutcome.tradeoff.excitingSlug === winner.machine.slug ? winner.machine : runnerUp.machine;
    return `
      <article class="panel dual-ready-strip">
        <h3>Both machines are ready</h3>
        <p class="muted">This choice now depends on priority: lower regret vs higher excitement.</p>
        <div class="dual-ready-grid">
          <div>
            <p class="eyebrow">Safer choice</p>
            <p><strong>${machineDisplayTitle(safer)}</strong></p>
            <p class="muted">Higher resale confidence, lower ownership effort, and steadier cost profile.</p>
          </div>
          <div class="dual-ready-center">Tradeoff: stability vs excitement</div>
          <div>
            <p class="eyebrow">More exciting choice</p>
            <p><strong>${machineDisplayTitle(exciting)}</strong></p>
            <p class="muted">Stronger theme pull and long-session engagement potential.</p>
          </div>
        </div>
        <p class="muted">Both are Ready. This choice is about preference, not eligibility.</p>
      </article>
    `;
  })() : "";

  const tieBreakerPanel = pairOutcome.tooClose ? `
    <article class="panel tie-breaker-panel">
      <p class="eyebrow">Tie-breaker</p>
      <h3>Resolve the final gap in 3 questions</h3>
      <p class="muted">${tieBreakerRationale(tieQuestions)}</p>
    ...
    return `
      <article class="panel compare-decision-card">
        ...
        <div class="card-actions">
          <button class="btn btn-primary small" type="button" data-action="choose-front-runner:${item.machine.slug}">Choose This as My Front-Runner</button>
  ...
  root.innerHTML = `
    <div class="compare-page-stack">
      <article class="panel compare-intro">
        ...
        <h2>Choose between your top options using your priorities, not full specs.</h2>
        <p class="muted">Compare is contextual to your shortlist and decision progress. Use one priority mode, then commit a front-runner.</p>
      </article>
      ...
      <article class="panel compare-priority-panel">
        ...
        <p class="muted">${PRESET_EXPLANATIONS[presetMode]}</p>
        <p class="muted"><strong>Why ranking changed:</strong> ${reasonLine}</p>
      </article>
      ...
      <details class="accordion" id="compare-deep-details" ${compareMode === "deep-details" ? "open" : ""}>
        <summary>Show Deep Details</summary>
        <div class="accordion__body">
          <p class="muted">Use deep details only if you still need extra confirmation after Pick One Now.</p>
...
root?.addEventListener("click", (event) => {
  ...
  if (action === "compare-auto-add") {
    if (autoAddTopTwoFromDecision()) render();
    return;
  }
  ...
  if (action.startsWith("choose-front-runner:")) {
    ...
    const selected = selectedMachinesFromSlugs(compareStore.get());
...
const decisionBar = initDecisionBar({
  page: "compare",
