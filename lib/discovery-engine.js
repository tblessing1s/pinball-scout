...
import { buildTraitSignalsFromReactionEntry, buildTraitSignalsFromTasteAnswers, experienceWeightFor } from "./refinement-helpers.js";
import { buildEvidenceModel, mismatchSignalsFor } from "./refinement-evidence.js";
import { buildTraitSignalsFromPlayedMachines, findRelatedMachines } from "../data/machine-components.js";
...
function applyTraitSignals(profile, traitSignals = {}, weight = 1) {
  if (!traitSignals || typeof traitSignals !== "object") return;
  Object.entries(traitSignals).forEach(([key, value]) => {
    if (!(key in profile.traitWeights)) return;
    profile.traitWeights[key] += Number(value || 0) * weight;
  });
}
...
function buildTasteProfile(answers = {}) {
  const profile = {
    ...
    tagWeights: {},
    traitWeights: Object.fromEntries(TRAIT_KEYS.map((key) => [key, 0]))
  };
  ...
  applyTraitSignals(profile, buildTraitSignalsFromTasteAnswers(answers), 1);
...
function buildProfile(reactions) {
  ...
  reactions.forEach((reactionEntry) => {
    ...
    const experienceWeight = reactionWeight(reactionEntry) * experienceWeightFor(reactionEntry.experienceType);
    const factor = reactionFactor(reaction) * experienceWeight;

    if (machine) {
      profile.machineScores[machineId] = (profile.machineScores[machineId] || 0) + (reaction === "looks-fun" ? 4 : reaction === "not-for-me" ? -4 : 0.6) * experienceWeight;
      ...
      TRAIT_KEYS.forEach((key) => {
        profile.traitWeights[key] += factor * traitSignal(machine.trait_profile?.[key]);
      });
    ...
    applyTraitSignals(profile, buildTraitSignalsFromReactionEntry(reactionEntry), experienceWeight);
    applyTraitSignals(profile, reactionEntry.traitSignals, experienceWeight);
  });
...
function profileScore(machine, profile) {
  ...
  TRAIT_KEYS.forEach((key) => {
    score += profile.traitWeights[key] * traitSignal(machine.trait_profile?.[key]) * 1.05;
  });
...
function tasteProfileScore(machine, tasteProfile) {
  ...
  TRAIT_KEYS.forEach((key) => {
    score += (tasteProfile.traitWeights?.[key] || 0) * traitSignal(machine.trait_profile?.[key]);
  });
...
function traitAlignment(machine, profile) {
  if (!machine || !profile) return [];
  const labelMap = traitLabelMap();
  const scored = TRAIT_KEYS
    .map((key) => ({
      key,
      label: labelMap[key] || key,
      score: (profile.traitWeights[key] || 0) * traitSignal(machine.trait_profile?.[key])
    }))
...
export function recommendMachines(context, reactions, limit = 3, options = {}) {
  const pool = buildDiscoveryPool(context);
  const refinementSignals = Array.isArray(options.refinementSignals) ? options.refinementSignals : [];
  const profile = buildProfile([...reactions, ...refinementSignals]);
  const tasteProfile = buildTasteProfile(context.tasteAnswers);
  const machineLocationIndex = options.machineLocationIndex || new Map();
  const evidenceModel = buildEvidenceModel(reactions, refinementSignals);

  // Boost profile from component signals of machines the user has played.
  // This uses structural knowledge (what features machines share) to pre-inform
  // the taste profile even before the user has rated each played machine.
  const playedSlugs = (options.playedMachineIds || [])
    .map((id) => discoveryMachineIndex.get(id)?.slug)
    .filter(Boolean);
  if (playedSlugs.length > 0) {
    const componentSignals = buildTraitSignalsFromPlayedMachines(playedSlugs);
    applyTraitSignals(profile, componentSignals, 0.85);
  }
  ...
  const ranked = pool
    .map((machine) => {
      ...
      return {
        ...
        traitAlignment: traitAlignment(machine, profile),
        mismatchSignals: mismatchSignalsFor(machine, evidenceModel)
      };
    })
...
export function recommendationConfidence(context, reactions = [], recommendations = [], options = {}) {
  const refinementSignals = Array.isArray(options.refinementSignals) ? options.refinementSignals : [];
  const evidenceModel = buildEvidenceModel(reactions, refinementSignals);
  ...
  const playedCount = evidenceModel.counts.played || 0;
  const videoCount = evidenceModel.counts.video || 0;
  const externalCount = evidenceModel.counts.external || 0;
  const conflicts = evidenceModel.conflicts.length;
  const topMismatchSignals = mismatchSignalsFor(recommendations[0], evidenceModel);
  const strongPositives = evidenceModel.strongPositives.length;
  const strongNegatives = evidenceModel.strongNegatives.length;

  let score = 0;
  if (practicalReady) score += 1;
  if (uniqueReactions >= 5) score += 2;
  else if (uniqueReactions >= 3) score += 1;
  if (playedCount >= 2) score += 1.8;
  else if (playedCount >= 1) score += 1.1;
  if (videoCount >= 1) score += 0.4;
  if (externalCount >= 1) score += 0.3;
  if (liked >= 2) score += 1;
  ...
  if (conflicts >= 2) score -= 1.2;
  else if (conflicts >= 1) score -= 0.6;
  if (topMismatchSignals.length) score -= 0.8;
  if (strongPositives >= 2 && conflicts === 0) score += 0.6;
  else if (strongPositives >= 1 && conflicts === 0) score += 0.3;
  if (strongNegatives >= 2 && conflicts === 0) score += 0.2;
  ...
  if (playedCount === 0) reasons.push("You have not logged a real-world play test yet.");
  if (conflicts >= 2) reasons.push("Some of your feedback points in conflicting directions.");
  if (topMismatchSignals.length) reasons.push("Your top pick still has a potential mismatch to validate.");
  ...
  if (videoCount > 0 && playedCount === 0) {
    nextSteps.push({
      id: "play-test",
      action: "open-nearby",
      label: "Log one real play test",
      detail: "Played-machine feedback should outweigh video-only reactions."
    });
  }
  if (!hasZip) {
    nextSteps.push({
  ...
  if (playedCount === 0 && !nextSteps.find((step) => step.id === "play-test")) {
    nextSteps.push({
      id: "real-play",
      action: "open-nearby",
  ...
  const confidenceDrivers = {
    ...
    externalCount,
    conflicts,
    mismatchSignals: topMismatchSignals,
    strongPositives,
    strongNegatives
  };
