...
import { buildTraitSignalsFromPlayedMachines } from "../data/machine-components.js";
...
// Maps component trait keys → recommendation signal/attribute keys with relative weights.
// Used to incorporate played machine component data into the taste profile.
const traitToSignalMap = {
  pace:              { fast_flowy: 1.0 },
  shot_satisfaction: { fast_flowy: 0.5, strategic_deeper_rules: 0.4 },
  rules_depth:       { strategic_deeper_rules: 1.0 },
  beginner_friendly: { beginner_friendly: 1.0 },
  theme_integration: { theme_first: 0.7, modern_immersive: 0.6 },
  chaos_level:       { chaotic_multiball_heavy: 1.0 },
  replayability:     { strategic_deeper_rules: 0.6, fast_flowy: 0.3 },
  wow_factor:        { modern_immersive: 0.7, chaotic_multiball_heavy: 0.4 }
};

// Nudge tasteProfile signal weights using the component characteristics of played machines.
// The scale is intentionally gentle (0.4×) so real reactions still dominate.
function applyComponentTraitBoost(tasteProfile, playedSlugs) {
  const traitSignals = buildTraitSignalsFromPlayedMachines(playedSlugs);
  Object.entries(traitSignals).forEach(([trait, value]) => {
    const signalMap = traitToSignalMap[trait];
    if (!signalMap) return;
    Object.entries(signalMap).forEach(([signalKey, weight]) => {
      if (signalKey in tasteProfile.signalWeights) {
        tasteProfile.signalWeights[signalKey] += value * weight * 0.4;
      }
    });
    if (trait === "beginner_friendly") {
      tasteProfile.attributeWeights.beginnerFriendly += value * 0.4;
    }
  });
}
...
export function recommendMachines(context, reactions, limit = 3, options = {}) {
  ...
  const tasteProfile = buildTasteProfile(context.tasteAnswers);
  if (options.playedMachineIds?.length) {
    const playedSlugs = options.playedMachineIds
      .map((id) => discoveryMachineIndex.get(id)?.slug)
      .filter(Boolean);
    if (playedSlugs.length) applyComponentTraitBoost(tasteProfile, playedSlugs);
  }
