...
export function recommendMachines(context, reactions, limit = 3, options = {}) {
  ...
  const playedIds = new Set(options.playedMachineIds || []);
  ...
  const ranked = pool
    .map((machine) => {
      ...
      return {
        ...machine,
        ...practical,
        recommendationScore: baseContextScore(machine, context) + tasteProfileScore(machine, tasteProfile) + profileScore(machine, profile) + practicalScore(machine, context, machineLocationIndex) + (playedIds.has(machine.id) ? 1.5 : 0),
